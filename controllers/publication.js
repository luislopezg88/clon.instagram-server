...
async function publish(file, ctx) {
    ...
    // genera el nombre con un id unico que no se repite
    const fileName = `publication/${uuidv4()}.${extension}`;
    const fileData = createReadStream();

    //subir imagen a amazon
...
async function getPublications(username) {
    ...
    if(!user) throw new Error("Usuarios no encontrado");
...
async function getPublicationsFolloweds(ctx) {
    ...
    for await (const data of followedsList) {
       ...
       const publications = await Publication.find().where({ idUser: data._id }).sort({ createAt: -1 })
        .populate("idUser");
